...
def insert_data_to_sql(data, table, cluster_value):
    ...
    if data:
        try:
            ...
            with connection.cursor() as cursor:
                insert_query = f"""
                INSERT INTO {table} (title, tags, description, system_time, computer_name, user_id, event_id, provider_name, dbscan_cluster, ip_address, task, rule_level, target_user_name, target_domain_name, ruleid, raw, unique_hash)
                VALUES (%s, %s, %s, %s, REPLACE(%s, ' ', ''), REPLACE(%s, ' ', ''), %s, %s, %s, %s, %s, %s, REPLACE(%s, ' ', ''), %s, %s, %s, SHA2(CONCAT_WS('|', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s), 256))
