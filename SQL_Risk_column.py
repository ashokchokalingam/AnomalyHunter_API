...
def insert_data_to_sql(data, table):
    ...
    if data:
        try:
            ...
            with connection.cursor() as cursor:
                insert_query = f"""
                INSERT INTO {table} (title, tags, description, system_time, computer_name, user_id, event_id, provider_name, dbscan_cluster, ip_address, task, rule_level, target_user_name, target_domain_name, ruleid, raw, unique_hash, risk)
                VALUES (%s, %s, %s, %s, REPLACE(%s, ' ', ''), REPLACE(%s, ' ', ''), %s, %s, NULL, %s, %s, %s, REPLACE(%s, ' ', ''), %s, %s, %s, SHA2(CONCAT_WS('|', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s), 256), NULL)
                ON DUPLICATE KEY UPDATE
                title = VALUES(title), tags = VALUES(tags), description = VALUES(description), computer_name = VALUES(computer_name), user_id = VALUES(user_id), event_id = VALUES(event_id), provider_name = VALUES(provider_name), dbscan_cluster = VALUES(dbscan_cluster), ip_address = VALUES(ip_address), task = VALUES(task), rule_level = VALUES(rule_level), target_user_name = VALUES(target_user_name), target_domain_name = VALUES(target_domain_name), ruleid = VALUES(ruleid), raw = VALUES(raw);
                """
                ...
                for i in range(0, len(data), BATCH_SIZE):
                    ...
                    cursor.executemany(insert_query, data_with_nulls)
                    connection.commit()
                logger.info(f"Inserted {len(data)} rows into '{table}' with dbscan_cluster as NULL.")
...
def process_and_insert_log(file_name, last_processed_time):
    ...
    if data:
        insert_data_to_sql(data, 'sigma_alerts')
    return latest_time
...
if __name__ == "__main__":
    ...
    ensure_column_exists("sigma_alerts", "risk", "INT")
