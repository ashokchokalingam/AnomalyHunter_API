...
def categorize_event(row, is_anomaly):
    """Generate a simplified event category based on the title."""
    title = row[1].lower()
...
def update_cluster_labels_and_categories(data, anomaly_labels):
    """Update the sigma_alerts table with the anomaly labels and event categories."""
    try:
        connection = mysql.connector.connect(**db_config)
        with connection.cursor() as cursor:
            update_query = """
            ...
            connection.commit()
            logging.info(f"Updated {len(update_data)} records with anomaly labels and event categories.")
    except Error as e:
        logging.error(f"Error updating anomaly labels and event categories: {e}")
    finally:
        if connection.is_connected():
            connection.close()
...
def detect_anomalies():
    ...
    update_cluster_labels_and_categories(data, anomaly_labels)
